      ...
      try {
        ...
        const fi  = (cands) => {
          for (const c of cands) {
            let idx = hdr.findIndex(h => h === c.toLowerCase());
            if (idx !== -1) return idx;
            idx = hdr.findIndex(h => h.replace(/\s+/g,'') === c.toLowerCase().replace(/\s+/g,''));
            if (idx !== -1) return idx;
          }
          return -1;
        };

        const iName    = fi(['الاسم','name']);
        const iNat     = fi(['رقم الهوية','الهوية','nationalid','national id']);
        const iPhone   = fi(['الجوال','الهاتف','phone','mobile']);
        const iEmp     = fi(['جهة العمل','employer','company']);
        const iPay     = fi(['طريقة الدفع','paymentmethod','payment method']);
        const iHasArr  = fi(['متأخرات','عليه متأخرات','has arrears']);
        const iArrAmt  = fi(['قيمة المتأخرات','arreamount','arrears amount']);
        const iLastPay = fi(['آخر سداد','last payment','lastpayment']);
        const iPayCnt  = fi(['عدد مرات السداد','paymentcount','payment count']);
        const iJoin    = fi(['تاريخ الانضمام','joindate','join date']);
        const iNotes   = fi(['الملاحظات','notes','ملاحظات']);
        const iIdFile  = fi(['اسم ملف الهوية','اسم ملف نسخة الهوية','id file']);
        const iCertFile= fi(['اسم ملف الشهادة','certificate file']);

        const newStudents = rows.slice(1).filter(r => Array.isArray(r) && r.length > 0).map(r => {
          const nat   = String(r[iNat] ?? '').replace(/\D/g,'').slice(0,10);
          const phone = normPhone(String(r[iPhone] ?? ''));
          const hasArr = iHasArr !== -1 ? (r[iHasArr] === 'نعم' || r[iHasArr] === 'yes') : false;
          let pay = String(r[iPay] ?? 'شبكة').trim();
          if (pay === 'كاش') pay = 'نقدي';
          return {
            ref: nextRef++,
            name: String(r[iName] ?? ''),
            nationalId: nat, phone,
            employer: String(r[iEmp] ?? ''),
            ...
            paymentMethod: pay, hasArrears: hasArr,
            arrearsAmount: hasArr ? String(r[iArrAmt] ?? '0') : '0',
            lastPayment:   iLastPay !== -1 ? String(r[iLastPay] ?? '') : '',
            paymentCount:  Number(iPayCnt !== -1 ? (r[iPayCnt] ?? 0) : 0),
            joinDate:      iJoin    !== -1 ? String(r[iJoin]    ?? '') : '',
            notes:         iNotes   !== -1 ? String(r[iNotes]   ?? '') : '',
          };
        });

        students.push(...newStudents);
        saveStorage(); renderTable();
        Swal.fire({ icon:'success', title:'تم الاستيراد', text:`تم استيراد ${newStudents.length} سجل.` });
      } catch (err) {
        console.error(err);
        Swal.fire({ icon:'error', title:'خطأ', text:'تعذّر قراءة الملف.' });
      }
      importFile.value = '';
    };
